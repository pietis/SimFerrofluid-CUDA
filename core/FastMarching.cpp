...
namespace Pivot {
	...
	void FastMarching::Solve(GridData<double> &phi, int maxSteps) {
	...
	void FastMarching::UpdateNeighbors(Vector3i const &coord, GridData<std::int8_t> const &visited, GridData<double> &tent, Heap &heap) {
		for (int i = 0; i < Grid::GetNumNeighbors(); i++) {
	...
	double FastMarching::SolveEikonalEquation(Vector3i const &coord, GridData<std::int8_t> const &visited, GridData<double> const &tent) {
		Vector3d tempPhi = Vector3d::Ones() * std::numeric_limits<double>::infinity();
