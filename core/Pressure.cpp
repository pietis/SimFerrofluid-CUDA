			...
			for (int i = 0; i < Grid::GetNumNeighbors(); i++) {
				...
				if (weight > 0) {
					...
					if (c >= 0) {
						...
						if (m_PressureJump) div += m_PressureJump(axis, face, side > 0 ? theta : 1 - theta) * weight * intfCoef;
					}
	...
	void Pressure::ApplyProjection(
		...
		ParallelForEach(velocity.GetGrids(), [&](int axis, Vector3i const &face) {
			...
			if (id0 >= 0 && id1 >= 0) {
				...
				double const pIntf  = m_PressureJump ? m_PressureJump(axis, face, theta) : 0.;
