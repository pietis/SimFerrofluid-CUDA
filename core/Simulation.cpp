...
void Simulation::Export(std::filesystem::path const &dirname,
                        bool initial) const {
    { // Export the contour
    ...
    if (initial) { // Export the collider
        ...
        ForEach(m_LevelSet.GetGrid(), [&](Vector3i const &cell) {
            if (m_SGrid.IsInsideCell(cell) && m_Collider.IsInside(cell))
                cnt++;
        });
        IO::Write(fout, cnt);
        ForEach(m_LevelSet.GetGrid(), [&](Vector3i const &cell) {
            if (m_SGrid.IsBoundaryCell(cell) || !m_Collider.IsInside(cell))
                return;
        ...
        ForEach(m_LevelSet.GetGrid(), [&](Vector3i const &cell) {
            ...
            Vector3d const pos = m_LevelSet.GetGrid().PositionOf(cell);
            Vector3d const n =
                TriLerp::Interpolate(m_Collider.GetNormal(), pos).normalized();
