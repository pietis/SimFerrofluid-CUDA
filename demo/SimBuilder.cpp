...
SimBuilder::BuildDipole(SimBuildOptions const &options) {
    ...
    Vector3d DPPos = Vector3d(0, - length * 1.0, 0);
    // Vector3d DPPos;
    if(options.EnableMag){
        sim->m_FieldApplied = [HextFactor, DPOrient, DPPos, length](const Vector3d& pos, double time) -> Vector3d{
            ...
            // Vector3d dppos = Vector3d::Unit(1) * length * (-1.2 + (-0.6 - (-1.2)) * timeFactor);
    ...
    // CSG::Union(sim->m_LevelSet, ImplicitBox(sgrid.GetDomainOrigin() + Vector3d(1, 0, 1) * length * (0.5 - 0.3),
    //                                 Vector3d(1, 0, 1) * length * 0.6 + Vector3d(0, 1, 0) * length * 0.06));
    // CSG::Union(sim->m_LevelSet,
    //            ImplicitPlane(sgrid.GetDomainOrigin() +
